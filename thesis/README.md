...
│   
├── latexmkrc
├── output.xmpdata → rinominato in thesis.xmpdata (deve chiamarsi come il file .tex principale, "thesis")
└── thesis.tex
```
...
- `thesis.xmpdata` (chiamato `output.xmpdata` nel template originale, ma **pdfx cerca sempre un file `<nome-del-.tex-principale>.xmpdata`**, quindi va rinominato in `thesis.xmpdata` per far coincidere il nome con `thesis.tex`): contiene i metadati della tesi, come titolo, autore, parole chiave ecc. Dovete modificarlo in base alla vostra tesi; non accetta le variabili definite in `config/variables.tex`, perché viene letto da pdfx prima che quel file sia caricato.
